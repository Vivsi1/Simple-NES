...
Cartridge::Cartridge(const std::string &filename)
{
    ...
    {
    case 0:
        mapper = std::make_unique<Mapper000>(header.prg_rom_chunks, header.chr_rom_chunks);
        break;
    case 1:
        mapper = std::make_unique<Mapper001>(header.prg_rom_chunks, header.chr_rom_chunks);
        break;
    default:
        std::cerr << "Mapper " << (int)mapperID << " not supported!\n";
        return;
    }
...
{
    uint32_t mapped_addr;
    if (mapper->ppuMapWrite(addr,data, mapped_addr) && mapped_addr < vCHRMemory.size())
    {
